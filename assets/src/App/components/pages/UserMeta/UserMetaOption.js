...
import {deleteUserMetaOption, updateUserMetaOptionLabel, updateUserMetaOptionValue} from "../../../redux/actions/userMetaStateActions";

const UserMetaOption  = ( {id, position, boxId, fieldId, dragHandle}) => {

    // manage global state
    const dispatch = useDispatch();
    const {values} = useSelector(state => state.userMetaStateReducer);
    ...
    const handleChangeOptionLabel = (l) => {
        dispatch(updateUserMetaOptionLabel(id, boxId, fieldId, l));
    };

    const handleChangeOptionValue = (v) => {
        dispatch(updateUserMetaOptionValue(id, boxId, fieldId, v));
    };
            ...
            <Modal title={`Confirm deleting field "${fetchedLabel}"`} visible={modalVisible}>
                ...
                <p>
                    <a
                        ...
                        onClick={(e) => {
                            e.preventDefault();
                            dispatch(deleteUserMetaOption(boxId, fieldId, id));
                        }}
                    >
                        Yes
                    </a>
