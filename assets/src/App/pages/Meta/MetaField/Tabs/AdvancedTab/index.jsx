import React from "react";
import PropTypes from 'prop-types';
import useTranslation from "../../../../../hooks/useTranslation";
import {useFormContext, useWatch} from "react-hook-form";
import Select from "../../../../../components/Forms/Select";
import InputHidden from "../../../../../components/Forms/InputHidden";
import {v4 as uuidv4} from "uuid";
import Label from "../../../../../components/Forms/Label";
import Input from "../../../../../components/Forms/Input";
import {isEmpty} from "../../../../../utils/objects";
import SelectMulti from "../../../../../components/Forms/SelectMulti";
import {
    isFileField,
    isNestableField,
    isRadioField,
    isTextualField,
    isUOMField,
    isURLField
} from "../../../../../utils/fields";
import {fieldTypes} from "../../../../../constants/fields";

const AdvancedTab = ({field, formId: baseFormId}) => {

    const { control, register, formState: {errors}, setValue, clearErrors } = useFormContext();

    const watchedFieldChildren = useWatch({
        control,
        name: baseFormId("children")
    });

    const watchedBlocks = useWatch({
        control,
        name: baseFormId("blocks")
    });

    const headlineOptions = [
        {label: useTranslation('On the top'), value: 'top'},
        {label: useTranslation('On the left'), value: 'left'},
        {label: useTranslation('On the right'), value: 'right'},
        {label: useTranslation('No display'), value: 'none'},
    ];

    const layoutOption = [
        { label: useTranslation("Row"), value: "row"},
        { label: useTranslation("Block"), value: "block"},
        { label: useTranslation("Table"), value: "table"},
    ];

    const yesOrNoOptions = [
        {label: useTranslation("Select"), value: null},
        {label: useTranslation("Yes"), value: "1"},
        {label: useTranslation("No"), value: "0"},
    ];

    let leadingFieldOptions = [];

    watchedFieldChildren && watchedFieldChildren.length > 0 && watchedFieldChildren.map((f) => {
        if(isTextualField(f.type)){
            leadingFieldOptions.push({
                label: f.label ? f.label : f.name,
                value: f.id
            });
        }
    });

    watchedBlocks && watchedBlocks.length > 0 && watchedBlocks.map((b) => {
        b.fields && b.fields.length > 0 && b.fields.map((f) => {
            if(isTextualField(f.type)){
                leadingFieldOptions.push({
                    label: f.label ? f.label : f.name,
                    value: f.id
                });
            }
        });
    });

    /**
     *
     * @param type
     * @param index
     * @param innerKey
     * @param label
     * @param placeholder
     * @param options
     * @return {*}
     */
    const renderAdvancedField = ({type, index, innerKey, label, placeholder, options}) => {

        /**
         *
         * @param value
         * @return {string}
         */
        const formId = (value) => {
            return `${baseFormId(`advancedOptions.${index}.${value}`)}`;
        };

        /**
         *
         * @return {null|*}
         */
        const option = () => {
            if(!field.advancedOptions){
                return null;
            }

            const filteredValue = field.advancedOptions[index];

            if(filteredValue){
                return filteredValue;
            }

            return null;
        };

        /**
         *
         * @return {{}}
         */
        const integerValidation = () => {
            let ob = {};

            if(options.min){
                ob.min =  {
                    value: options.min,
                    message: useTranslation("min length is") + " " + options.min
                };
            }

            if(options.max){
                ob.max =  {
                    value: options.max,
                    message: useTranslation("max length is") + " " + options.max
                };
            }

            return ob;
        };

        return (
            <React.Fragment>
                <InputHidden
                    id={formId("id")}
                    value={option() ? option().id : uuidv4()}
                    register={register}
                />
                <InputHidden
                    id={formId("key")}
                    value={option() ? option().key : innerKey}
                    register={register}
                />
                <Label
                    id={formId("value")}
                    label={<span dangerouslySetInnerHTML={{__html: label}}/>}
                />
                {type === 'integer' && (
                    <Input
                        type="number"
                        id={formId("value")}
                        register={register}
                        errors={errors}
                        placeholder={placeholder}
                        min={options.min ? options.min : null}
                        max={options.max ? options.max : null}
                        step={options.step ? options.step : null}
                        defaultValue={option() ? option().value : null}
                        validate={integerValidation()}
                    />
                )}
                {type === 'select' && options && options.options && (
                    <React.Fragment>
                        {options.multiple && options.multiple === true ? (
                            <SelectMulti
                                clearErrors={clearErrors}
                                errors={errors}
                                id={formId("value")}
                                register={register}
                                setValue={setValue}
                                values={options.options}
                                defaultValue={option() ? option().value : null}
                            />
                        ) : (
                            <Select
                                register={register}
                                id={formId("value")}
                                errors={errors}
                                placeholder={placeholder}
                                values={options.options}
                                defaultValue={option() ? option().value : null}
                            />
                        )}
                    </React.Fragment>
                )}
                {type === 'text' && (
                    <Input
                        id={formId("value")}
                        register={register}
                        errors={errors}
                        placeholder={placeholder}
                        defaultValue={option() ? option().value : null}
                    />
                )}
            </React.Fragment>
        );
    };

    return (
        <div className="flex-column s-24">
            <fieldset className="acpt-fieldset">
                <legend>
                    {useTranslation("General settings")}
                </legend>
                <div className="flex-column s-24">
                    <div className="container align-end">
                        <div className="col-6">
                            {renderAdvancedField({
                                type: "select",
                                index: 1,
                                innerKey: "headline",
                                placeholder: useTranslation("Select the headline location (default: on the left)"),
                                label: useTranslation("Headline location"),
                                options: {
                                    options: headlineOptions
                                }
                            })}
                        </div>
                        <div className="col-6">
                            {renderAdvancedField({
                                type: "integer",
                                index: 2,
                                innerKey: "width",
                                placeholder: useTranslation("Field width in % ([1-100] default value: 100)"),
                                label: useTranslation("Field width"),
                                options: {
                                    min: 1,
                                    max: 100,
                                    step: 1
                                }
                            })}
                        </div>
                    </div>
                    <div className="container align-end">
                        <div className="col-6">
                            {renderAdvancedField({
                                type: "text",
                                index: 6,
                                innerKey: "before",
                                placeholder: useTranslation("This text will appear before input"),
                                label: useTranslation("Text before. HTML is allowed (ex. &lt;small&gt;)"),
                                options: {}
                            })}
                        </div>
                        <div className="col-6">
                            {renderAdvancedField({
                                type: "text",
                                index: 7,
                                innerKey: "after",
                                placeholder: useTranslation("This text will appear after input"),
                                label: useTranslation("Text after. HTML is allowed (ex. &lt;small&gt;)"),
                                options: {}
                            })}
                        </div>
                    </div>
                    <div className="container align-end">
                        <div className="col-12">
                            {renderAdvancedField({
                                type: "text",
                                index: 18,
                                innerKey: "css",
                                placeholder: useTranslation("These CSS classes will be applied to back-end editor fields"),
                                label: useTranslation("Add CSS classes to back-end editor fields (ex. class-active pulse)"),
                                options: {}
                            })}
                        </div>
                    </div>
                </div>
            </fieldset>
            {isTextualField(field.type) && (
                <fieldset className="acpt-fieldset">
                    <legend>
                        {useTranslation("Textual fields settings")}
                    </legend>
                    <div className="flex-column s-24">
                        <div className="container align-end">
                            <div className="col-6">
                                {renderAdvancedField({
                                    type: "integer",
                                    index: 8,
                                    innerKey: "min",
                                    placeholder: useTranslation("The minimum value (used only by numeric and textual fields)"),
                                    label: useTranslation("Min value (used only by numeric and textual fields)"),
                                    options: {}
                                })}
                            </div>
                            <div className="col-6">
                                {renderAdvancedField({
                                    type: "integer",
                                    index: 9,
                                    innerKey: "max",
                                    placeholder: useTranslation("The maximum value (used only by numeric and textual fields)"),
                                    label: useTranslation("Max value (used only by numeric and textual fields)"),
                                    options: {}
                                })}
                            </div>
                        </div>
                        <div className="container align-end">
                            <div className="col-6">
                                {renderAdvancedField({
                                    type: "integer",
                                    index: 10,
                                    innerKey: "step",
                                    placeholder: useTranslation("Step value (used only by numeric fields)"),
                                    label: useTranslation("Step value (used only by numeric fields)"),
                                    options: {}
                                })}
                            </div>
                            <div className="col-6">
                                {renderAdvancedField({
                                    type: "text",
                                    index: 11,
                                    innerKey: "pattern",
                                    placeholder: useTranslation("Pattern value (used only by numeric and textual fields)"),
                                    label: useTranslation("Pattern value (used only by textual fields)"),
                                    options: {}
                                })}
                            </div>
                        </div>
                    </div>
                </fieldset>
            )}
            {isFileField(field.type) && (
                <fieldset className="acpt-fieldset">
                    <legend>
                        {useTranslation("File field settings")}
                    </legend>
                    <div className="flex-column s-24">
                        <div className="container align-end">
                            <div className="col-6">
                                {renderAdvancedField({
                                    type: "select",
                                    index: 14,
                                    innerKey: "accepts",
                                    placeholder: useTranslation("Input file accepts"),
                                    label: useTranslation("Input file accepts"),
                                    options: {
                                        multiple: true,
                                        options: [
                                            { label: "image", value: "image" },
                                            { label: "video", value: "video" },
                                            { label: "document", value: "document" },
                                            { label: "audio", value: "audio" },
                                        ]
                                    }
                                })}
                            </div>
                            <div className="col-6">
                                {renderAdvancedField({
                                    type: "select",
                                    index: 5,
                                    innerKey: "hide_url_label",
                                    placeholder: useTranslation("Hide the label"),
                                    label: useTranslation("Hide the label"),
                                    options: {
                                        options: yesOrNoOptions
                                    }
                                })}
                            </div>
                        </div>
                        <div className="container align-end">
                            <div className="col-6">
                                {renderAdvancedField({
                                    type: "integer",
                                    index: 16,
                                    innerKey: "max_size",
                                    placeholder: useTranslation("Maximum size"),
                                    label: `${useTranslation("Maximum size")} (Mb)`,
                                    options: {}
                                })}
                            </div>
                            <div className="col-6">
                                {renderAdvancedField({
                                    type: "integer",
                                    index: 17,
                                    innerKey: "min_size",
                                    placeholder: useTranslation("Minimum size"),
                                    label: `${useTranslation("Minimum size")} (Mb)`,
                                    options: {}
                                })}
                            </div>
                        </div>
                    </div>
                </fieldset>
            )}
            {isRadioField(field.type) && (
                <fieldset className="acpt-fieldset">
                    <legend>
                        {useTranslation("Radio field settings")}
                    </legend>
                    <div>
                        {renderAdvancedField({
                            type: "select",
                            index: 4,
                            innerKey: "hide_blank_radio",
                            placeholder: useTranslation("Hide the blank value"),
                            label: useTranslation("Hide the blank value"),
                            options: {
                                options: yesOrNoOptions
                            }
                        })}
                    </div>
                </fieldset>
            )}
            {isURLField(field.type) && (
                <fieldset className="acpt-fieldset">
                    <legend>
                        {useTranslation("URL field settings")}
                    </legend>
                    <div>
                        {renderAdvancedField({
                            type: "select",
                            index: 5,
                            innerKey: "hide_url_label",
                            placeholder: useTranslation("Hide the label"),
                            label: useTranslation("Hide the label"),
                            options: {
                                options: yesOrNoOptions
                            }
                        })}
                    </div>
                </fieldset>
            )}
            {isNestableField(field.type) && (
                <fieldset className="acpt-fieldset">
                    <legend>
                        {useTranslation("Nestable fields layout settings")}
                    </legend>
                    <div className="flex-column s-24">
                        <div className="container align-end">
                            <div className="col-6">
                                {renderAdvancedField({
                                    type: "select",
                                    index: 19,
                                    innerKey: "leading_field",
                                    placeholder: useTranslation("The leading field (which will be displayed when the repeater is collapsed)."),
                                    label: useTranslation("Leading field"),
                                    options: {
                                        options: leadingFieldOptions
                                    }
                                })}
                            </div>
                            <div className="col-6">
                                {renderAdvancedField({
                                    type: "select",
                                    index: 20,
                                    innerKey: "layout",
                                    placeholder: useTranslation("The look of the field for admins and editors."),
                                    label: useTranslation("Field layout"),
                                    options: {
                                        options: layoutOption
                                    }
                                })}
                            </div>
                        </div>
                        <div className="container align-end">
                            <div className="col-4">
                                {renderAdvancedField({
                                    type: "integer",
                                    index: 3,
                                    innerKey: "columns",
                                    placeholder: useTranslation("Field columns ([1-6] only for repeater field)"),
                                    label: useTranslation("Field columns"),
                                    options: {
                                        min: 1,
                                        max: 6,
                                        step: 1
                                    }
                                })}
                            </div>
                            <div className="col-4">
                                {renderAdvancedField({
                                    type: "integer",
                                    index: 12,
                                    innerKey: "minimum_blocks",
                                    placeholder: useTranslation("The limit on how many elements are required ([1-100] only for repeater field)"),
                                    label: useTranslation("Minimum elements allowed"),
                                    options: {}
                                })}
                            </div>
                            <div className="col-4">
                                {renderAdvancedField({
                                    type: "integer",
                                    index: 13,
                                    innerKey: "maximum_blocks",
                                    placeholder: useTranslation("The limit on how many elements are allowed ([1-100] only for repeater field)"),
                                    label: useTranslation("Maximum elements allowed"),
                                    options: {}
                                })}
                            </div>
                        </div>
                    </div>
                </fieldset>
            )}
            {isUOMField(field.type) && (
                <fieldset className="acpt-fieldset">
                    <legend>
                        {useTranslation("Unit of measure fields settings")}
                    </legend>
                    <div>
                        {renderAdvancedField({
                            type: "select",
                            index: 0,
                            innerKey: "uom_default_value",
                            placeholder: useTranslation("UOM default value"),
                            label: useTranslation("UOM default value"),
                            options: {
                                options: document.globals.globals.uom[field.type.toLowerCase()]
                            }
                        })}
                    </div>
                </fieldset>
            )}
        </div>
     );
};

AdvancedTab.propTypes = {
    formId: PropTypes.func.isRequired,
    boxIndex: PropTypes.number.isRequired,
    fieldIndex: PropTypes.number.isRequired,
    field: PropTypes.object.isRequired,
};

export default AdvancedTab;