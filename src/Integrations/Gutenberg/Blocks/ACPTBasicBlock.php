	...
	private function getRawData($field, $attributes = [])
	{
		...
		switch ($field['type']){
			...
			case MetaFieldModel::ADDRESS_MULTI_TYPE:
				...
				return Maps::renderMulti($width, $height, $zoom, $rawData, true);
			...
			case MetaFieldModel::DATE_RANGE_TYPE:
				...
				if(is_array($rawData) and !empty($rawData) and count($rawData) === 2){
					...
					if( !empty($attributes['dateFormat']) and Date::isDateFormatValid($attributes['dateFormat']) ){
						$value = date_i18n($attributes['dateFormat'], strtotime($rawData[0]));
						$value .= ' - ';
						$value .= date_i18n($attributes['dateFormat'], strtotime($rawData[1]));
			...
			case MetaFieldModel::TIME_TYPE:
				...
				if( !empty($attributes['timeFormat']) and Date::isDateFormatValid($attributes['timeFormat']) ){
					return  date_i18n($attributes['timeFormat'], strtotime($rawData));
				}
			...
			case MetaFieldModel::URL_TYPE:
				...
				return '<a style="'.$style.'" href="'.Url::sanitize($url).'" target="'.$target.'">'.$label.'</a>';
